...
public class Produtos {
        ...
        switch (opcao) {
            ...
            case 7:
                if (grupoNome.equals("cliente")) {
                    ...
                    //Converte o ID do usuário (perfilId)
                    int clienteId = Controller.Cliente.getClienteIdByUsuarioId(perfilId);

                    if (clienteId > 0) {
                        // Chama a função com o ID CORRETO do cliente
    ...
    // Adiciona itens ao carrinho, N linhas para N produtos
    public static void adicionarAoCarrinho(Scanner sc, int clienteId) {
        ...
        try (Connection con = Conexao.getConnection()) {
            ...
            // Assumimos que a tabela 'carrinho' tem as colunas 'cliente_id' e 'produto_cod'
            ...
            try (PreparedStatement stmt = con.prepareStatement(sqlInsert)) {
                ...
                for (int i = 0; i < quantidade; i++) {
                    stmt.setInt(1, clienteId);
                    stmt.setString(2, produtoCod);
                    stmt.addBatch(); // Adiciona ao lote para inserção eficiente
                    sucesso++;
                }

                // Executa o lote de inserts
            ...
            // 4. Reduzir o estoque (o produto saiu da prateleira e foi para o carrinho)
            String sqlUpdateStock = "UPDATE produtos SET estoque = estoque - ? WHERE COD = ?";
            try(PreparedStatement psUpdate = con.prepareStatement(sqlUpdateStock)){
                psUpdate.setInt(1, quantidade);
                psUpdate.setString(2, produtoCod);
            ...
            if (e.getErrorCode() == 1452) {
                System.out.println("Verifique se o COD do produto está correto e se o cliente ID é válido.");
            }
        }
    }
